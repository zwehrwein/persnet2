...
row_to_tidygraph <- function(df_row_input) {
  ...
  psn_edges_variables <- select(df_row_input, tie1:a_tie105)
  ...
  psn_name_cols <- select(df_row_input, c(
  ...
  psn_keep_name_cols <- select(df_row_input, name_1:name_15)
  ...
  if (!is.null(psn_edges) && is.matrix(psn_edges) && nrow(psn_edges) > 0) {
    ...
    tgra <- as_tbl_graph(psn_el, directed = FALSE) %>%
      mutate(record_id = df_row_input$record_id,  # Add record ID
             node_id = row_number())  # Create a unique node ID
    ...
    tgra <- tbl_graph(
      ...
      activate(nodes) %>%
      mutate(record_id = df_row_input$record_id,
             node_id = row_number())
