...
#'
#' @examples
#' plot_network(tidygra)
plot_network <- function(tidygra) {
  ##########
  ...
  edge_attributes <- tidygra %>% activate(edges) %>% as_tibble()
  if (!"weight" %in% colnames(edge_attributes)) {
    ...
    node_names <- unique(tidygra %N>% pull(name))
    
    # Transform tie strength into strong/weak strings and create an alter dummy variable
    tidygra <- tidygra %>%
      activate(edges) %>%
      mutate(strength_of_tie = ifelse(weight == 1, "weak", "strong")) %>%
      activate(nodes) %>%
      mutate(alter_dummy = ifelse(name != 'ego', 1, 0))
