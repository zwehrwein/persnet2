...
calc_node_ens <- function(tidygra, node_index = NULL) {
  ...
  if (dim(as_adjacency_matrix(tidygra))[1] < 2) return(NA)
  
  # Check if the edge attribute 'weight' is missing
  edge_attrs <- tidygra %>% activate(edges) %>% as_tibble()
  if (!"weight" %in% colnames(edge_attrs)) {
    ...
    tidygra <- tidygra %>%
      activate(edges) %>%
      mutate(weight = 1)  # Add default weight of 1
  }
  
  # Convert to adjacency matrix
  adj_matrix <- as_adjacency_matrix(tidygra, attr = "weight", sparse = FALSE)
  node_names <- V(tidygra)$name  # Extract node names from graph
