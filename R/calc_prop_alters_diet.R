...
#'
#' @examples
#' calc_prop_alters_diet(persnet_row)
calc_prop_alters_diet <- function(persnet_row) {
  ...
  if (length(diet_cols_string) == 0) {
    stop("Error: Cannot find variables related to whether alters had a good diet.")
  }
  
  # Select diet-related columns
  diet_cols <- persnet_row %>% select(name1diet:name15diet)
